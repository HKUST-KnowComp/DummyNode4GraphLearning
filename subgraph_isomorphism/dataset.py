...
class LRPDataset(Dataset):
    ...
    @staticmethod
    def generate_neighbour_perms(adj_m, start_node, dummy_nodes=None):
        ...
        elif start_node in dummy_nodes:
            all_perm = combinations(adjlist, min(nei_len, len(adjlist)))
            return tuple((start_node,) + tuple(sorted(p)) for p in all_perm) + tuple((start_node,) + tuple(sorted(p)[::-1]) for p in all_perm)
        else:
            exist_dummy_nodes = list()
            new_adjlist = list()
            for n in adjlist:
                if n in dummy_nodes:
                    exist_dummy_nodes.append(n)
                else:
                    new_adjlist.append(n)
            if len(exist_dummy_nodes) == 0:
                all_perm = permutations(adjlist, min(nei_len, len(adjlist)))
                return tuple((start_node,) + p for p in all_perm)
            else:
                all_perm = list()
                nei_len -= 1
                for dummy_node in exist_dummy_nodes:
                    all_perm.extend([p + (dummy_node,) for p in permutations(new_adjlist, min(nei_len, len(new_adjlist)))])
                return tuple((start_node,) + p for p in all_perm)
    ...
    def graph_to_egonet_seq(graph):
        ...
        if DUMMYFLAG in graph.ndata:
            dummy_nodes = set(th.nonzero(graph.ndata[DUMMYFLAG] == 1).flatten().tolist())
        else:
            dummy_nodes = None
        for i in range(num_of_nodes):
            perms = LRPDataset.generate_neighbour_perms(adj_m, start_node=i, dummy_nodes=dummy_nodes)
