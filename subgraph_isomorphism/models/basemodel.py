...
class EdgeSeqModel(BaseModel):
    ...
    def forward(self, pattern, graph):
        ...
        # handle the dummy edges
        if DUMMYFLAG in pattern.tdata:
            p_dummy_mask = pattern.tdata[DUMMYFLAG].view(bsz, -1, 1)
            p_mask = p_mask.masked_fill(p_dummy_mask, 0)
            del p_dummy_mask
        if DUMMYFLAG in graph.tdata:
            g_dummy_mask = graph.tdata[DUMMYFLAG].view(bsz, -1, 1)
            g_mask = g_mask.masked_fill(g_dummy_mask, 0)
            del g_dummy_mask
...
class GraphAdjModel(BaseModel):
    ...
    def forward(self, pattern, graph):
        ...
        # handle the dummy nodes
        if DUMMYFLAG in pattern.ndata:
            p_v_dummy_mask = pattern.ndata[DUMMYFLAG].view(-1, 1)
            p_v_mask = p_v_mask.masked_fill(split_and_batchify_graph_feats(p_v_dummy_mask, p_v_len, pre_pad=True)[0], 0)
            del p_v_dummy_mask
        if DUMMYFLAG in graph.ndata:
            g_v_dummy_mask = graph.ndata[DUMMYFLAG].view(-1, 1)
            g_v_mask = g_v_mask.masked_fill(split_and_batchify_graph_feats(g_v_dummy_mask, g_v_len, pre_pad=True)[0], 0)
            del g_v_dummy_mask
...
class GraphAdjModelV2(BaseModel):
    ...
    def forward(self, pattern, graph):
        ...
        if DUMMYFLAG in pattern.ndata:
            ...
            p_v_mask = p_v_mask.masked_fill(split_and_batchify_graph_feats(p_v_dummy_mask, p_v_len, pre_pad=True)[0], 0)
            del p_v_dummy_mask
        if DUMMYFLAG in pattern.edata:
            p_e_dummy_mask = (pattern.edata[DUMMYFLAG]).view(-1, 1)
            p_e_mask = p_e_mask.masked_fill(split_and_batchify_graph_feats(p_e_dummy_mask, p_e_len, pre_pad=True)[0], 0)
            del p_e_dummy_mask
        ...
        if DUMMYFLAG in graph.edata:
            g_e_dummy_mask = (graph.edata[DUMMYFLAG]).view(-1, 1)
            g_e_mask = g_e_mask.masked_fill(split_and_batchify_graph_feats(g_e_dummy_mask, g_e_len, pre_pad=True)[0], 0)
            del g_e_dummy_mask
