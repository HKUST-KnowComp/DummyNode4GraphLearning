...
def process_model_config(config):
    ...
    # for dummy nodes
    # the number of nodes adds one
    # the number of node labels adds one
    # for dummy edges
    # the number of edges adds the number of original nodes
    # the number of edge labels adds two
    if config["add_dummy"]:
        model_config["max_nge"] += config["max_ngv"] * 2
        model_config["max_npe"] += config["max_npv"] * 2
        model_config["max_ngel"] += 2
        model_config["max_npel"] += 2
        model_config["max_ngv"] += 1
        model_config["max_npv"] += 1
        model_config["max_ngvl"] += 1
        model_config["max_npvl"] += 1
...
def build_model(config, **kw):
    ...
    elif config["rep_net"] == "HGT":
        model = HGT(pred_return_weights=config["match_weights"], **config, **kw)
    return model
...
def add_dummy_nodes_edges(dataset, max_npv, max_npvl, max_npe, max_npel, max_ngv, max_ngvl, max_nge, max_ngel):
    if isinstance(dataset, EdgeSeqDataset):
        for x in dataset:
            if DUMMYFLAG not in x["graph"].tdata:
                num_gv = x["graph"].number_of_nodes()
                u = th.arange(num_gv)
                v = th.LongTensor([max_ngv]).repeat(num_gv)
                ul = dict()
                ul.update(zip(x["graph"].tdata["u"].numpy(), x["graph"].tdata["ul"].numpy()))
                ul.update(zip(x["graph"].tdata["v"].numpy(), x["graph"].tdata["ul"].numpy()))
                ul = th.LongTensor([ul.get(i, max_ngvl) for i in range(num_gv)])
                vl = th.LongTensor([max_ngvl]).repeat(num_gv)
                el = th.LongTensor([max_ngel]).repeat(2 * num_gv)
                el[num_gv:] += 1
                x["graph"].add_tuples(
                    th.cat([u, v], dim=0),
                    th.cat([v, u], dim=0),
                    ...
                    th.cat([vl, ul], dim=0),
                    data={
                        DUMMYFLAG: th.ones((num_gv * 2,), dtype=th.bool),
                        REVFLAG: th.cat([th.zeros((num_gv,), dtype=th.bool), th.ones((num_gv,), dtype=th.bool)])
                    }
                )

                if x["counts"] > 0:
                    x["subisomorphisms"] = th.cat(
                        [
                            x["subisomorphisms"],
                            th.tensor([[num_gv]]).repeat(x["counts"], 1)
                        ], dim=1
                    )

            if DUMMYFLAG not in x["pattern"].tdata:
                num_pv = x["pattern"].number_of_nodes()
                u = th.arange(num_pv)
                v = th.LongTensor([max_npv]).repeat(num_pv)
                ul = dict()
                ul.update(zip(x["pattern"].tdata["u"].numpy(), x["pattern"].tdata["ul"].numpy()))
                ul.update(zip(x["pattern"].tdata["v"].numpy(), x["pattern"].tdata["ul"].numpy()))
                ul = th.LongTensor([ul.get(i, max_npvl) for i in range(num_pv)])
                vl = th.LongTensor([max_npvl]).repeat(num_pv)
                el = th.LongTensor([max_npel]).repeat(2 * num_pv)
                el[num_pv:] += 1
                x["pattern"].add_tuples(
                    th.cat([u, v], dim=0),
                    th.cat([v, u], dim=0),
                    ...
                    th.cat([vl, ul], dim=0),
                    data={
                        DUMMYFLAG: th.ones((num_pv * 2,), dtype=th.bool),
                        REVFLAG: th.cat([th.zeros((num_pv,), dtype=th.bool), th.ones((num_pv,), dtype=th.bool)])
                    }
                )
    elif isinstance(dataset, GraphAdjDataset):
        for x in dataset:
            if DUMMYFLAG not in x["graph"].edata:
                num_gv = x["graph"].number_of_nodes()
                num_ge = x["graph"].number_of_edges()
                u = th.arange(num_gv)
                v = th.LongTensor([num_gv]).repeat(num_gv)
                # eid = th.arange(max_nge, max_nge + 2 * num_gv)
                eid = th.LongTensor([max_nge]).repeat(2 * num_gv)
                eid[num_gv:] += 1
                el = th.LongTensor([max_ngel]).repeat(2 * num_gv)
                el[num_gv:] += 1
                x["graph"].add_nodes(
                    1,
                    data={
                        NODEID: th.LongTensor([max_ngv]),
                        NODELABEL: th.LongTensor([max_ngvl]),
                        DUMMYFLAG: th.ones((1,), dtype=th.bool)
                    }
                )
                x["graph"].add_edges(
                    th.cat([u, v], dim=0),
                    ...
                    data={
                        EDGEID: eid,
                        EDGELABEL: el,
                        DUMMYFLAG: th.ones((2 * num_gv,), dtype=th.bool),
                        REVFLAG: th.cat([th.zeros((num_gv,), dtype=th.bool), th.ones((num_gv,), dtype=th.bool)])
                    }
                )
                if REVFLAG in x["graph"].edata:
                    x["graph"].edata[REVFLAG][-num_gv:].fill_(1)
            ...
            if DUMMYFLAG not in x["pattern"].edata:
                num_pv = x["pattern"].number_of_nodes()
                num_pe = x["pattern"].number_of_edges()
                u = th.arange(num_pv)
                v = th.LongTensor([num_pv]).repeat(num_pv)
                # eid = th.arange(max_npe, max_npe + 2 * num_pv)
                eid = th.LongTensor([max_npe]).repeat(2 * num_pv)
                eid[num_pv:] += 1
                el = th.LongTensor([max_npel]).repeat(2 * num_pv)
                el[num_pv:] += 1
                x["pattern"].add_nodes(
                    1,
                    data={
                        NODEID: th.LongTensor([max_npv]),
                        NODELABEL: th.LongTensor([max_npvl]),
                        DUMMYFLAG: th.ones((1,), dtype=th.bool)
                    }
                )
                x["pattern"].add_edges(
                    ...
                    data={
                        EDGEID: eid,
                        EDGELABEL: el,
                        DUMMYFLAG: th.ones((2 * num_pv,), dtype=th.bool),
                        REVFLAG: th.cat([th.zeros((num_pv,), dtype=th.bool), th.ones((num_pv,), dtype=th.bool)])
                    }
                )
                if REVFLAG in x["pattern"].edata:
                    x["pattern"].edata[REVFLAG][-num_pv:].fill_(1)
...
def convert_to_conjugate(dataset):
    if isinstance(dataset, EdgeSeqDataset):
        for x in dataset:
            p = x["pattern"].to_graph()
            g = x["graph"].to_graph()
            conj_p = convert_conjugate_graph(p)
            conj_g = convert_conjugate_graph(g)
            ...
            if x["counts"] > 0 and p.number_of_edges() > 0:
                ...
                conj_subisomorphisms = get_conjugate_subisomorphisms(
        ...
        for x in dataset:
            conj_p = convert_conjugate_graph(x["pattern"])
            conj_g = convert_conjugate_graph(x["graph"])
            # find the corresponding edge isomorphisms
...
if __name__ == "__main__":
    ...
    # add one dummy node and 2*N dummy edges
    if config["add_dummy"]:
        if logger:
            logger.info("adding dummy nodes and edges...")
        for data_type in datasets:
            add_dummy_nodes_edges(datasets[data_type], max_npv, max_npvl, max_npe, max_npel, max_ngv, max_ngvl, max_nge, max_ngel)
        max_npe += 2 * max_npv
        max_npel += 2
        max_npv += 1
        max_npvl += 1
