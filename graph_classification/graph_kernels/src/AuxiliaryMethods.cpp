...
GraphDatabase read_graph_txt_file(string path, string data_set_name)
{
    ...
    ifstream myfile(path + "/" + data_set_name + "/" + data_set_name + "_graph_indicator.txt");
    if (myfile.is_open())
    {
    ...
    string label;
    Labels node_labels;
    ifstream labels(path + "/" + data_set_name + "/" + data_set_name + "_node_labels.txt");
    if (labels.is_open())
    {
    ...
    ifstream attributes(path + "/" + data_set_name + "/" + data_set_name + "_node_attributes.txt");
...
vector<int> read_classes(string path, string data_set_name)
{
    ...
    ifstream myfile(path + "/" + data_set_name + "/" + data_set_name + "_graph_labels.txt");
    if (myfile.is_open())
    {
        while (getline(myfile, line))
        {
...
vector<vector<float>> read_multi_targets(string path, string data_set_name)
{
    ...
    ifstream myfile(path + "/" + data_set_name + "/" + data_set_name + "_graph_attributes.txt");
    if (myfile.is_open())
    {
        while (getline(myfile, line))
        {
...
void write_libsvm(const GramMatrix &gram_matrix, const vector<int> classes, string filename, const bool normalize)
{
    ...
    if (file.is_open())
    {
        for (int i = 0; i < size; i++)
        {
            ...
            {
                file << " " << (c + 1) << ":" << dense_gram_matrix(i, c);
            }
            file << std::endl;
        }
        file.close();
