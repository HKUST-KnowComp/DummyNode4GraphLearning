...
{
ShortestPathKernel::ShortestPathKernel(const GraphDatabase &graph_database)
    : m_graph_database(graph_database), m_distance_to_index(), m_num_distances(0)
{
}
    ...
    for (Node k = 0; k < num_nodes; ++k)
    {
        for (Node i = 0; i < num_nodes; ++i)
        {
            for (Node j = i; j < num_nodes; ++j)
            {
                ...
                {
                    if (shortest_path_distances[i][j] > shortest_path_distances[i][k] + shortest_path_distances[k][j])
                    {
