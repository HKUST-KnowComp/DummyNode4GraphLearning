...
class GraphletKernel
{
  public:
    ...
    GramMatrix compute_gram_matrix(const bool use_node_labels, const bool use_edge_labels, const bool compute_gram);

    ~GraphletKernel();

  private:
    // Computes number of graphlets in graph.
    GraphletCounter compute_graphlet_count(const Graph &g, const bool use_node_labels, const bool use_edge_labels);
